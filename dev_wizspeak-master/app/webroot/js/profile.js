    ...
    $('#about_me_edit').on('click','.share:button', function(){
        ...
        $('#about_me_edit').css({'display': 'none'});
        $('#about_me_labels').css({'display':'block'});
        var data = {"user_id" : jsVars.wall_id, "status" : newtext, "is_mentor" :jsVars.is_mentor};
		console.log(data);
  ...
  //Personal Details Update
    $("#personal_details_edit").on("click", '.common-button', function() {
		...
		console.log(data);
	        new callAjax(data,'users/update_personal_details','afterUserDetUpdate').fire();
    ...
    $(".education-desc").on("click", '.common-button', function() {
    	...
    	if($(this).val() == 'SAVE' && $('#education_id').val() && formId == "eduAdd") {
    	...
    	if($(this).val() == 'SAVE' && formId == "expAdd" ) {
			...
			$('#'+formId+"List").each(function(){
                this.reset();
            });
    	}


    	if($(this).val() == 'SAVE' && $('#award_id').val() && $(this).attr("id") == 'addAwar') {
    	...
    	if($(this).val() == 'SAVE' && $('#certification_id').val() && $(this).attr("id") == 'addCer') {
             ...
             var formData = new FormData($('#user_education_id_'+form_id)[0]);
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        ...
                        div = div+'<div class="editPencil-remove trash" data="edu"  title="Delete" id="'+form_id+'">&nbsp;</div>';
                        div = div+result;
    ...
    $('#edit_group_members').on('click','.black-pencil', function(){
       ...
       table = '<div class="mentor-row" id="addDiv_'+user.value+'"  style="width:51%;" >'+
        ...
        $('.left-container-profile').on('click','.trash', function(){
			...
			if(confirm(" are you really wanr to delete ")){
	...
	function afterUserDetUpdate(result){
