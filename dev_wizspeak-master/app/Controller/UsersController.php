...
public $components = array( 'Paginator', 'Session', 'Cookie', 'Log', 'Curl', 'Crop');
 ...
 */
        ...
        public function beforeFilter() {
            ...
            $this->Auth->allow('registration', 'logout','account','homePage','opauth_complete');
                ...
                if ($this->Auth->login()) {
                    ...
                    $this->_setCookie($this->Auth->user('id'));
                            ...
                            if ($this->Auth->user('is_mentor') == 0) {
                                ...
                                $this->redirect(array(
                                    'controller' => 'mentor',
                                    'action' => $this->Auth->user('userId'),'about'));
                            }
		...
		protected function accountType($name,$value){
			$data = array(
				$name => $value
			);
			$this->Cookie->write('UserType', $data, true, time() + (60 * 10));
			return true;
        ...
        public function registration() {
			...
			if(isset($this->data['auth'])){
				...
				switch($this->data['auth']['provider']){
					case 'Facebook':
					...
					case 'Twitter':
						...
						$easyUser = array(
							'first_name' => $this->data['auth']['info']['name'],
							'last_name' => '',
							'email' => $this->data['auth']['info']['email'],
							'uid' => $this->data['auth']['uid'],
							'provider' => 'twitter',
			...
			if($this->request->is("post") AND (!empty($this->request->data['account']))){
				$this->accountType('UserType',$this->request->data['account']);


			}
			//debug($this->Cookie->read('UserType.UserType'));
			if(empty($this->Cookie->read('UserType.UserType')) ){

				$this->redirect(array("controller" => 'users', 'action' => 'account'));
			}
			if($this->Cookie->read('UserType.UserType') == 'mentor'){
				$userType = 1;
			}
			elseif($this->Cookie->read('UserType.UserType') == 'user'){
				$userType = 0;
			}else{
				$this->redirect(array("controller" => 'users', 'action' => 'account'));
			}
            ...
            if ($this->request->is('post') AND (!empty($this->request->data['User']))) {


				if($this->data['User']['gender']==1){
					$gender = 'male';
				}
				if($this->data['User']['gender']==0){
					$gender = 'female';
				}
				$easyUser = array(
                ...
                $this->request->data['User']['userId'] = md5($this->request->data['User']['email']);
                $this->request->data['User']['activated'] = 1;
                $this->request->data['User']['activated'] = 1;
				if(empty($this->request->data['User']['country'])){

					$this->request->data['User']['country'] = $this->request->data['User']['countryName'];
				}
                ...
                if ($this->User->save($this->request->data)) {
					...
					if($userType==1){
						//mentor
						$this->redirect(['action' => 'select_expertise']);
					}
					if($userType==0){
						$this->redirect(['action' => 'select_ambition']);
					}
...
//debug($this->request->data);
                        ...
                        if(isset($this->request->data['ambitions'])){
							...
							if(!empty($this->request->data['ambitions'])){
								...
								foreach($ids as $sc_id){
									...
									$this->UserCategoryRelations->create();
                        ...
                        if(isset($this->request->data['hobbies'])){
							...
							if(!empty($this->request->data['hobbies'])){
        ...
        public function select_expertise(){
			...
			if (empty($this->Auth->user()))
			{
				if($this->Cookie->read('UserType.UserType') == 'user'){
            ...
            $this->loadModel('UserCategoryRelations');
                    if ($this->request->is('post')) {
                         ...
                         $status = TRUE;
                        if(isset($this->request->data['experty'])){

                            $ids = explode(',', $this->request->data['experty']);


							if(!empty($this->request->data['experty'])){
		...
		public function homePage(){
			...
			if($this->Auth->user('is_mentor')){
				//is mentor
				$this->redirect(Array('controller' => 'mentor','action' => $this->Auth->user('userId'),'about'));
			}else{
				//is user
				$this->redirect(Array('controller' => 'ambitions','action' => 'index'));
			}
			die("user home page redirect");
			//do redirect from here
        ...
        public function profile_photo() {
			...
			$this->set('avathar' ,'');
                if(!empty($query)){
                     ...
                     $this->set('avathar' , IMAGE_CHANGE.$path['link']);
                }
                   ...
                   if (!empty($this->request->data['Post']['doc_file']['tmp_name']) && is_uploaded_file($this->request->data['Post']['doc_file']['tmp_name'])) {
                       ...
                       $allowedExts = array("image/gif", "image/jpeg", "image/jpg", "image/png", "image/JPG", "image/JPEG");
                          ...
                          // $pic_name = $this->crop_image($file_name, $this->request->data['Post']);
						   ...
						   $img_name = $this->Crop->profile_crop($file_name, $this->request->data['Post'],$this->Auth->user("userId"));
						   ...
						   $result = $this->Curl->fetchCurl("/addUserProfilePic/".$img_name."/".$this->Auth->user("userId"));
                           ...
                           $this->set('avathar' , IMAGE_CHANGE.$img_name);
        ...
        public function update_personal_details() {
            ...
            if($this->request->is('ajax')){
                if($this->request->data['user_id'] == $this->Auth->user('userId')){
