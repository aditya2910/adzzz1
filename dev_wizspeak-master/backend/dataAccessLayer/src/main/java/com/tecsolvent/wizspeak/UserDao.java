...
public class UserDao {
	...
	public String getCoverPic(long wallId, int wallType, int picType) throws Exception {
		...
		try {
			...
			pic  = slaveJdbcTemplate.query("select link from user_group_profile_pics where wall_id = "+wallId+" AND wall_type = "+wallType+" AND is_avatar = "+picType+" AND is_active = 1 ", new getCoverPicMapper());
		}
		catch( Exception e) {
			logger.error("Error in getting getCoverPic pic "+e);
		}

		return pic;
	}

	private class getCoverPicMapper implements ResultSetExtractor<String> {
		public String extractData(ResultSet resultSet) throws SQLException {
